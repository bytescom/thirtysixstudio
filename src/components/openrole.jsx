...
const OpenRole = ({ isBgChange }) => {
    ...
    return (
        <div className='max-w-[98%] m-auto tracking-tight leading-tight my-[6rem]'>
            <div className={`cursor-pointer border-b-[2px] ${!isBgChange ? 'border-white' : 'border-[#fd2c2a]'}`}>
                <div
                    onClick={() => setIsOpen(!isOpen)}
                    className='flex justify-between items-center border-b-[1px] border-[#292929] py-2 pl-[22.2rem] pr-[36rem]'
                >
                    <h3>ROLES OPEN</h3>
                    <span className='border-gray-700 hover:text-white hover:bg-black border-[1px] rounded-2xl py-1 px-4'>
                        {!isOpen ? (
                ...
                >
                    {roles.map((rolesItem, index) => (
                        <p
                            key={index}
                            className="pl-[22.2rem] pr-[36rem] py-2 border-b border-[#292929]
